...
public class ReportsController : ApiControllerBase
{
    ...
    [HttpGet("s6-hkd")]
    [Authorize(Roles = "Owner,Manager")]
    public async Task<IActionResult> GetS6Ledger([FromQuery] DateTime startDate, [FromQuery] DateTime endDate, [FromQuery] int page = 1, [FromQuery] int pageSize = 20)
    {
        if (startDate == default)
            startDate = new DateTime(DateTime.UtcNow.Year, DateTime.UtcNow.Month, 1, 0, 0, 0, DateTimeKind.Utc);
        else
        ...
        var data = await _reportsService.GetCashLedgerAsync(startDate, endDate, BizFlow.Domain.Enums.PaymentMethod.Cash, null, page, pageSize);
        return Ok(data);
    }

    [HttpGet("s7-hkd")]
    [Authorize(Roles = "Owner,Manager")]
    public async Task<IActionResult> GetS7Ledger([FromQuery] DateTime startDate, [FromQuery] DateTime endDate, [FromQuery] Guid? bankAccountId = null, [FromQuery] int page = 1, [FromQuery] int pageSize = 20)
    {
        if (startDate == default)
            startDate = new DateTime(DateTime.UtcNow.Year, DateTime.UtcNow.Month, 1, 0, 0, 0, DateTimeKind.Utc);
        else
        ...
        var data = await _reportsService.GetCashLedgerAsync(startDate, endDate, BizFlow.Domain.Enums.PaymentMethod.Transfer, bankAccountId, page, pageSize);
        return Ok(data);
    }
