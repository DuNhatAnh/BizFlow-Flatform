...
export default function CategoryItem({
  ...
  return (
    <React.Fragment>
      <li 
        className="p-3 flex justify-between items-center hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0 group" 
        ...
        {isEditing ? (
          <div className="flex flex-col gap-2 w-full mr-2">
            <div className="flex gap-2 w-full">
              <input 
                ...
                className="flex-1 px-3 py-1.5 border border-slate-200 rounded-lg text-sm text-slate-800 min-w-[100px] focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                autoFocus
              />
              <select
                value={editParentId}
                onChange={e => setEditParentId(e.target.value === "" ? "" : Number(e.target.value))}
                className="px-3 py-1.5 border border-slate-200 rounded-lg text-sm max-w-[130px] text-slate-800 shrink-0 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all cursor-pointer"
              >
              ...
              <div className="flex gap-1.5 items-center flex-wrap px-1">
                {sortedColors.slice(0, 10).map((color) => (
                  <button
                    ...
                    className={`w-5 h-5 rounded-full transition-transform shrink-0 shadow-sm ${editColor === color ? 'ring-2 ring-offset-1 ring-primary scale-110' : 'hover:scale-110 opacity-70 hover:opacity-100'}`}
          ...
          <>
            <span className="text-sm font-medium text-slate-700 flex items-center gap-2">
              {level > 0 && <span className="w-3 border-b border-l h-4 inline-block -mt-4 border-slate-300 rounded-bl-sm"></span>}
              {node.color && (
                <span className="w-2.5 h-2.5 rounded-full shadow-sm" style={{ backgroundColor: node.color }}></span>
              )}
              {node.name}
            </span>
