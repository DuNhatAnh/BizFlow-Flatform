...
export default function StoreSettings({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  useEffect(() => {
    ...
    const savedConfig = localStorage.getItem("bizflow_dashboard_config");
    if (savedConfig) {
      try {
  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in duration-300">
      {/* Toast */}
      {toast && (
        <div className={`fixed top-6 left-1/2 -translate-x-1/2 z-[70] px-6 py-3 rounded-full shadow-lg border animate-in slide-in-from-top-4 flex items-center gap-3 ${toast.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-red-50 border-red-200 text-red-800'
          }`}>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-64 shrink-0">
          <div className="bg-white rounded-2xl border border-surface-container-high shadow-sm overflow-hidden flex flex-col">
            ...
            >
              <LayoutDashboard className="w-5 h-5" /> Tùy chỉnh Dashboard
            </button>
          </div>
        </div>
        ...
        <div className="flex-1">
          ...
          {activeTab === "dashboard" && (
            <div className="bg-white rounded-2xl border border-surface-container-high shadow-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              <div className="p-6 border-b border-surface-container-low bg-surface-container-lowest">
                <h3 className="font-bold text-lg text-on-surface">Tùy chỉnh hiển thị Dashboard</h3>
                <p className="text-sm text-on-surface-variant mt-1">Chọn thông tin muốn hiển thị trên màn hình tổng quan để tập trung vào điều quan trọng nhất.</p>
              </div>
              <div className="p-6 space-y-8">
                <div>
                  <h4 className="font-bold text-on-surface mb-4">Khoảng thời gian mặc định</h4>
                  <p className="text-sm text-on-surface-variant mb-4">Khoảng thời gian này sẽ tự động được chọn mỗi khi bạn truy cập trang Dashboard.</p>
                  <select
                    value={dashboardConfig.defaultRange}
                    onChange={(e) => setDashboardConfig({ ...dashboardConfig, defaultRange: e.target.value })}
                    className="w-full max-w-sm p-3 border border-outline-variant rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                  >
                ...
                <div>
                  <h4 className="font-bold text-on-surface mb-4">Thẻ chỉ số (Widgets)</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {[
                      { id: 'showRevenue', label: 'Doanh thu' },
                      { id: 'showGrossProfit', label: 'Lợi nhuận gộp' },
                      { id: 'showOrderCount', label: 'Số đơn hàng' },
                      { id: 'showNetCashFlow', label: 'Dòng tiền thuần' },
                      { id: 'showTotalExpenses', label: 'Tổng chi phí' },
                      ...
                      <label key={widget.id} className="flex items-center justify-between p-4 border border-surface-container-low rounded-xl cursor-pointer hover:bg-surface-container-lowest transition-colors">
                        ...
                        <div className="relative inline-flex items-center cursor-pointer">
                          <input
                            ...
                            checked={dashboardConfig[widget.id as keyof typeof dashboardConfig] as boolean}
                            onChange={(e) => setDashboardConfig({ ...dashboardConfig, [widget.id]: e.target.checked })}
                          />
                          <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:bg-primary transition-colors duration-200"></div>
                          <div className="absolute left-[2px] top-[2px] w-5 h-5 bg-white border border-gray-300 rounded-full transition-transform duration-200 peer-checked:translate-x-full peer-checked:border-white"></div>
                        </div>
                      </label>
                    ))}
                  </div>
